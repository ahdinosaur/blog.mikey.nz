...
- [x] Handle the `/[slug]/[asset]` vs `/assets/[asset]` slug collision. Reject reserved slugs (`assets`, `archives`, `atom.xml`) in `loadPost`.
- [x] `parseAssetUrl` (`src/lib/posts.ts:269-270`) silently ignores URLs with more than 2 path segments. Assert in `loadPost` when an in-post asset link doesn't fingerprint
- [x] Delete `normalizeFrontmatter` (`src/lib/posts.ts:303-309`) and fix the legacy posts that need it
