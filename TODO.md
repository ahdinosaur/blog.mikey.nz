...
- [ ] Fix `<img/>` regex in `src/lib/markdown.ts:89`. `/<img([^>]*)\/?>/gi` is greedy and on input `<img src="x"/>` produces the malformed `<img src="x"/ loading="lazy" />`. Use the same conservative form as `src/lib/posts.ts:156` — `/<img\b([^>]*?)\s*\/?>/gi`.
...
- [ ] Decide how to handle the `/[slug]/[asset]` vs `/assets/[asset]` slug collision. Either reject reserved slugs (`assets`, `archives`, `atom.xml`) in `loadPost`, or add a code comment documenting the constraint.
- [ ] `parseAssetUrl` (`src/lib/posts.ts:269-270`) silently ignores URLs with more than 2 path segments. Either assert in `loadPost` when an in-post asset link doesn't fingerprint, or document the constraint.
- [ ] Either delete `normalizeFrontmatter` (`src/lib/posts.ts:303-309`) and fix the legacy posts that need it, or add a comment naming which posts depend on it.
